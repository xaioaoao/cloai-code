/**
 * Marketplace manager for acode plugins
 *
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a acode bug, shouldn't hit the error file.
    return {}
  }
}
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `acode version. Run 'claude marketplace remove ${name}' and ` +
